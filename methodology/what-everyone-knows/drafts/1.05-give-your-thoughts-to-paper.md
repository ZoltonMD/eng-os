...
I decided to use the hybrid system that contains analog and digital notes which perform different functions, and compensate for each other's shortcomings. I use paper as RAM, for active thinking, inbox, daily to-dos; digital—storage: archive, references, long-term data with tagging and search. This approach allows me to take the best of both worlds; at the same time we have a flexible, accessible, but unreliable paper notebook in *runtime*, and reliable organized cold storage with predefined schema in case we need to store data that may be useful in the long term.

// --- In progress ---

When choosing a system, do not forget our rule about tool maintenance, we defined in chapter **1.03** talking about productive environment. I twice moved from one system to another, because they changed rules, subscription methods, etc. These moves take too much of your time and attention. And when I was doing that a second time I decided I had to stop this continuous migration. So, I've started using my own solution: local markdown files in a directory tree, which I sync between my phone and laptop.
